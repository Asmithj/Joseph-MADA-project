...
kable(
  ...
  format = "pandoc",
  caption = "*Table 2: Association Between Malaria Episodes and Risk of Preterm Birth*",
...
# Replace raw names with neat labels; replace any NA with blank
...
# Remove any row names so only the column appears in the final table
rownames(table_stillbirth_pars) <- NULL

# Display table with consistent styling using kable with format "pandoc"
kable(
  table_stillbirth_pars,
  format = "pandoc",
  caption = "*Table 3: Association Between Malaria Episodes and Risk of Stillbirth*",
  col.names = c("Exposure and Adjustment Variables", "Adjusted Odds Ratio (aOR)", "95% CI Lower", "95% CI Upper", "p-value")
) %>%
  kable_styling(full_width = FALSE, position = "center")
...
In adjusted logistic regression analysis (Table 4a), the number of malaria episodes during pregnancy was not significantly associated with preterm birth (aOR: 1.04; 95% CI: 0.95–1.14; p = 0.401). Similarly, the penalized Firth logistic regression model showed no significant association between malaria episodes and stillbirth (aOR: 0.96; 95% CI: 0.83–1.10; p = 0.559) (Table 4b). No significant associations were observed with age at enrollment, gravidity, or hypertension for either outcome.

*Effect of Treatment Arm on Malaria Episode Incidence*
A Poisson regression model assessing malaria episode incidence by treatment arm found no significant difference between the DP and SP treatment groups (IRR: 1.02; 95% CI: 0.92–1.12; p = 0.756) after adjusting for age, gravidity, and education level (Table 4c).

A summary of these primary outcome analyses is presented in Table 4.
