...
# pick only the model variables
md <- train_data[, c("gravidity", "age_at_enrollment_years", "total_malaria_episodes", "adverse_birth_outcome")]

dd <- datadist(md)
options(datadist = "dd")

# fit the logistic model
f <- lrm(adverse_birth_outcome ~ gravidity, data = md, x = TRUE, y = TRUE)

cal <- calibrate(f, method="boot", B = 200)
# then plot(cal, ...)
