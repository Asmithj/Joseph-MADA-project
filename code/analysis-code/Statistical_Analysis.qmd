...
# 0) Engineer features and impute their NAs
full_data <- promo_data_clean %>%
  mutate(
    enroll_month = month(enrollment_date),
    episodes_per_trimester = total_malaria_episodes /
      (gestational_age_at_enrollment_weeks / (40/3))
  )

# compute medians for imputation
median_enroll <- median(full_data$enroll_month, na.rm = TRUE)
median_eps    <- median(full_data$episodes_per_trimester, na.rm = TRUE)

full_data <- full_data %>%
  mutate(
    enroll_month           = if_else(is.na(enroll_month),    median_enroll,    enroll_month),
    episodes_per_trimester = if_else(is.na(episodes_per_trimester),
                                     median_eps,
                                     episodes_per_trimester)
  )

# 1) Safe stratification check
stratum_counts <- full_data %>% count(adverse_birth_outcome)
can_stratify   <- nrow(stratum_counts) >= 2 && all(stratum_counts$n >= 2)

# 2) 70/30 split (stratified if safe)
set.seed(1234)
split_obj <- if (can_stratify) {
  initial_split(full_data, prop = 0.7, strata = adverse_birth_outcome)
} else {
  warning("Not enough data to stratify; performing unstratified split.")
  initial_split(full_data, prop = 0.7)
}

raw_train <- training(split_obj)
raw_test  <- testing(split_obj)

# 3) Drop rows with NAs in the *core* six predictors or outcome
...
# 4) Build dynamic v-fold CV (up to 10, but ≤ nrow(train_data))
n_train <- nrow(train_data)
if (n_train < 2) stop("Not enough rows for cross‐validation (need ≥ 2).")
v <- min(10, n_train)

cv_folds <- if (can_stratify) {
  vfold_cv(train_data, v = v, strata = adverse_birth_outcome)
} else {
  vfold_cv(train_data, v = v)
}

# 5) Finally, tune your workflow—no more stray NAs in enroll_month
enet_tune <- tune_grid(
  enet_wf,
  resamples = cv_folds,
  grid      = enet_grid,
...
enet_tune <- tune_grid(
  ...
  resamples = cv_folds,  # the vfold_cv object you just built
  grid      = enet_grid, # the grid of penalty/mixture values
  metrics   = metric_set(roc_auc)
)

# (B) Look at the tuning results
collect_metrics(enet_tune) %>% 
  filter(.metric == "roc_auc") %>%
  arrange(desc(mean))

# (C) Pick the best parameters
best_params <- select_best(enet_tune, "roc_auc")
