...
# 1. Check the class of the variable
class(promo_data_clean$preterm_births_count)

# 2. View its internal structure
str(promo_data_clean$preterm_births_count)

# 3. Get basic summary statistics
summary(promo_data_clean$preterm_births_count)

# 4. Tabulate all observed values (including NAs)
table(promo_data_clean$preterm_births_count, useNA = "always")

# 5. Quick glimpse using dplyr (avoids calling select on a vector)
glimpse(promo_data_clean %>% select(preterm_births_count))
```
